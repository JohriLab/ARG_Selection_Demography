...
expansion_plot <- ggplot2::ggplot(expansion_Ne %>% mutate(Nesquish = oob_squish(expansion_Ne$Ne, range = c(ifelse(species == "human", 300, 3000), ifelse(species == "human", 100000, 10000000))))) +
  annotation_logticks(short = unit(0.05, "cm"), outside = F, sides = "l")+
  ggplot2::geom_line(ggplot2::aes(x = nGen, y = Nesquish, group = newcol, colour = pfix), alpha = 0.2) +
  ggplot2::geom_smooth(ggplot2::aes(x = nGen, y = Nesquish, group = pfix, colour = pfix), se = F) +
...
Figure4 <- 
  build_constantplot(
    ...
    colours = c("light blue", #"dark blue", 
                "gray", "#DBCF18", "#D87812", "#71A500", "#E11B1B"),
